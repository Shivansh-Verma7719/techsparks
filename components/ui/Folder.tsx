...
const Folder: React.FC<FolderProps> = ({ color = '#5227FF', size = 1, items = [], className = '' }) => {
  ...
  const getOpenTransform = (index: number) => {
    if (isMobile()) {
      if (index === 0) return 'translate(-140%, -80%) rotate(-15deg)';
      if (index === 1) return 'translate(30%, -80%) rotate(15deg)';
      if (index === 2) return 'translate(-50%, -155%) rotate(0deg)';
      return '';
    }
    else {
      if (index === 0) return 'translate(-150%, -80%) rotate(-15deg)';
      if (index === 1) return 'translate(55%, -80%) rotate(15deg)';
      if (index === 2) return 'translate(-50%, -130%) rotate(0deg)';
      return '';
    }
  };
